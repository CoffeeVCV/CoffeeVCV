...
struct Some2: Module {
    ...
    void process(const ProcessArgs & args) override {
        ...
        if (trigButtonPressed || trigTriggered) {
            ...
            int targetOutpts = (int(NUM_OUTS * p));
            ...
            for (int i = 0; i < NUM_OUTS; i++) {
                ...
                DEBUG("swaping %d with %d", i, j);
            }

            // activate outputs
            int c = 0;
            for (int i = 0; i < NUM_OUTS; i++) {
                if (outs[i] > -1 && c < targetOutpts) {
                    lights[L_ACTIVE + outs[i]].value = 1;
                    outputs[O_CV + outs[i]].setVoltage(inputs[I_CV].getVoltage());
                    c++;
                } else {
                    lights[L_ACTIVE + outs[i]].value = 0;
                    outputs[O_CV + outs[i]].setVoltage(0);
                }
            }
        }
        ...
        if (_lowPriority.process()) {
            for (int i = 0; i < NUM_OUTS; i++) {
                lights[L_SELECT + i].value = (i < selected) ? 1 : 0;
            }
        }
    }
};
