		...
		if (lowPriority.process())
		{
			...
			for (int i = 0; i < NUM_GROUPS; i++)
			{
				...
				if (button || trigger)
				{
					...
					{
						if (i == 0)
							params[P_G1 + row].setValue(nudge(params[P_G1 + row].getValue(), range, nudgev, -range, range, nudgeMode));
						if (i == 1)
							params[P_G2 + row].setValue(nudge(params[P_G2 + row].getValue(), range, nudgev, -range, range, nudgeMode));
						if (i == 2)
							params[P_G3 + row].setValue(nudge(params[P_G3 + row].getValue(), range, nudgev, -range, range, nudgeMode));
						if (i == 3)
							params[P_G4 + row].setValue(nudge(params[P_G4 + row].getValue(), range, nudgev, -range, range, nudgeMode));
					}
				}
			}
